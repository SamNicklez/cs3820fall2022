...
specialForms :: [String]
specialForms = ["if", "cond", "and", "or", "let*", "lambda","quote","splice","begin","set!","define"]
...
specialFormHandler "quote" [xs,Nil]  = splice  xs where
  splice (Cons (Symbol "splice") (Cons Nil Nil))  = specialFormHandler "splice" []
  splice (Cons (Symbol "splice") (Cons rest Nil)) = eval rest
  splice (Cons d Nil) = eval d
  --splice (Cons d (Cons (Symbol "splice") (Cons rest Nil))) = splice rest
  splice d = return d


specialFormHandler "splice" [] = fail "cannot splice ()"
specialFormHandler "splice" xs = eval (proper xs)

specialFormHandler "begin" xs = evalBegin xs where
  evalBegin :: [Datum] -> Result Datum
  evalBegin [x,Nil] = eval x
  evalBegin (x:xs) = do
    r <- eval x
    specialFormHandler "begin" xs
  evalBegin _ = fail "begin has failed"
...
--specialFormHandler "begin" [d,Nil] = eval d
--specialFormHandler "begin" (d1:d2:ds) = do
 -- eval d1
  --specialFormHandler "begin" $ d2:ds --recursive call should be on (d2:ds)
--specialFormHandler "begin" args = argsBorked "begin" args
...
-- >>> test "((lambda (x) (list ((lambda (x) (begin (set! x 5) x)) x) x)) 1)"
-- Right "(1 1)"
...
-- >>> test "$()"
-- Left Error: "I do not know how to evaluate (Cons nil (Cons nil nil))"

-- >>> specialFormHandler "splice" [(Symbol "t")]
-- No instance for (Show (Result Datum))
--   arising from a use of ‘evalPrint’

-- >>> (test "'(+ $(fst '(x . y)) $(snd '(y . z)))")
-- Right "(+ (splice (fst (quote (x . y)))) (splice (snd (quote (y . z)))))"
...
primHandler "eval" [xs,Nil] = eval xs
...
primOps :: Frame
primOps = map (\ s -> (s, PrimOp s (primHandler s))) primitives
...
{- eval (Begin []) = Error "Empty begin expression"
eval (Begin [x]) = eval x
eval (Begin (x:xs)) = eval (Begin xs) -}
...
test' :: String -> Datum
test' d = case eval_maybe (parse d) of
  Nothing -> test' (printDatum (parse d))
  Just da -> da
